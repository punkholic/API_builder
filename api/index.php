...
require 'vendor/autoload.php';
    ...
    Flight::route('/recommend', function () {
        ...
        for( $i = 2; $i < count( $files_list ); $i++ ) {
            ...
            // $points_percent = $points_count * 100 ;

           if( $points_count > 2 ) {
            $whitelisted_json[$files_list[$i]] = $points_count;
           }
        }
        ...
        if ( !empty( $whitelisted_json ) ) {
            ...
            if ( count( $whitelisted_json ) == 1  ) {              
                ...
                foreach( $whitelisted_json as $key => $json_points ) {
                    $smallest = $json_points;
                    $json_id = $key;
                    if ( $smallest > $json_points ) {
                        $smallest = $json_points;
                        $json_id = $key;
                    }
                }
